...
async def test_list_todos_filter_title_should_return_5_todos(
    ...
    response = client.get(
        '/api/v1/todos/?title=Test todo 1',
        headers={'Authorization': f'Bearer {token}'},
    )
...
async def test_list_todos_filter_combined_should_return_5_todos(
    ...
    response = client.get(
        '/api/v1/todos/?title=Test todo combined'
        '&description=combined&state=done',
        headers={'Authorization': f'Bearer {token}'},
    )
...
def test_delete_todo_error(client, token):
    response = client.delete(
        f'/api/v1/todos/{10}', headers={'Authorization': f'Bearer {token}'}
    )
